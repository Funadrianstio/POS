...
/*document.getElementById('calculate-btn').addEventListener('click', () => {
  // Grab input values and convert to float, defaulting to 0 if empty or NaN
  const fittingCopay = parseFloat(document.getElementById('fitting-copay').value) || 0;
  const lensAllowance = parseFloat(document.getElementById('lens-allowance').value) || 0;
  const fittingDiscountAmount = parseFloat(document.getElementById('fitting-discount-amount').value) || 0;
  const fittingDiscountPercent = parseFloat(document.getElementById('fitting-discount-percent').value) || 0;
  const irTraining = parseFloat(document.getElementById('ir-training').value) || 0;
  const additionalSavings = parseFloat(document.getElementById('additional-savings').value) || 0;

  // Calculate discount as amount + percent of fittingCopay
  const fittingDiscount = fittingDiscountAmount + (fittingCopay * (fittingDiscountPercent / 100));

  // Sum up all amounts minus discounts and savings
  const total = fittingCopay + lensAllowance + irTraining - fittingDiscount - additionalSavings;

  // Display the result (you can change this to update an output div if you want)
