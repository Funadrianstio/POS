...
function determineFittingFee(selections, fittingFeesData) {
  if (selections.fittingType === 'Sphere') {
    if (selections.selfPay === 'Yes') {
      console.log(`Fitting fee is ${fittingFeesData[0].selfpay}`);
    } else if (selections.selfPay === 'No') {
      if (selections.newEst === 'New') {
        console.log(`Fitting fee is ${fittingFeesData[0].insnew}`);
      } else if (selections.newEst === 'Est') {
        console.log(`Fitting fee is ${fittingFeesData[0].insestablished}`);
      } else {
        console.warn("Invalid value for selections.newEst");
      }
    } else {
      console.warn("Invalid value for selections.selfPay");
    }
  } else {
    console.log("Fitting type is not Sphere — no fitting fee calculated.");
  }
}