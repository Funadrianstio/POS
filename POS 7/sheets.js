  ...
  select.addEventListener('change', () => {
    ...
    if (brandData) {
      if (eye === 'right') {
        ...
        if (rebateDisplay) {
          updateRebateDisplay(rightEyeData);
        } else {
...
// Rebate function
function updateRebateDisplay(data) {
  const rebateDisplay = document.getElementById('rebateDisplay');
  if (!data || !rebateDisplay) {
    console.warn("Missing data or display element.");
    return;
  }

  const isNewToBrand = selections.newToBrand?.toLowerCase() === 'yes';

  let rebate = 0;

  if (isNewToBrand && data.rebatesfornewwearer) {
    rebate = parseFloat(data.rebatesfornewwearer) || 0;
  } else if (data.yearsupplycurrent) {
    rebate = parseFloat(data.yearsupplycurrent) || 0;
  }
